...
def reduce_dimension_ndc(df, ndc_df):
    ...
    '''
    df = pd.merge(df, ndc_df[['NDC_Code', 'Non-proprietary Name']], left_on='ndc_code', right_on='NDC_Code')
...
def patient_dataset_splitter(df, patient_key='patient_nbr'):
    ...
    train = df[df[patient_key].isin(patients[:int(0.6*len(patients))])].reset_index(drop=True)
    validation = df[df[patient_key].isin(patients[int(0.6*len(patients)):int(0.8*len(patients))])].reset_index(drop=True)
    test = df[df[patient_key].isin(patients[int(0.8*len(patients)):])].reset_index(drop=True)
